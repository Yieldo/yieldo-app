...
export function useWalletBalances() {
  ...
  const ethMainnet = useBalance({ address, chainId: mainnet.id, query: { enabled: isConnected, staleTime: 60_000 } });
  const ethBase = useBalance({ address, chainId: base.id, query: { enabled: isConnected, staleTime: 60_000 } });
  const ethArb = useBalance({ address, chainId: arbitrum.id, query: { enabled: isConnected, staleTime: 60_000 } });
  const ethOp = useBalance({ address, chainId: optimism.id, query: { enabled: isConnected, staleTime: 60_000 } });
  ...
  const { data: tokenResults } = useReadContracts({
    ...
    query: { enabled: isConnected && wagmiContracts.length > 0, staleTime: 60_000 },
  });
      ...
      tokenResults.forEach((res, i) => {
        ...
        if (res.status === "success" && res.result) {
          ...
          if (balance > 0.01) {
            ...
            tokenAgg[meta.symbol].usdValue += balance;
